...
//"       $ jans -N 3571082522473766674484304975778527401895200115726120795842576355509746402614775567 -F 6434337 -M 6500000\n"
//"       $ jans -N 1522605027922533360535618378132637429718068114961380688657908494580122963258952897654000350692006139 -F 44412127 -M 50000000\n"
" " << std::endl;
   ...
   while (( c = getopt_long( argc, argv, "hvN:F:M:Z:T:B:", long_options, &option_index )) != -1 ){
      switch( c ){
         ...
         case '?':
            ...
            break;
         case 'v':
            std::cout << "jans version 0.01" << std::endl;
            return 0;
            break;
         case 'N':
            temp_str = optarg;
            //std::cout << optarg[ temp_str.length() ] << std::endl;
            break;
         case 'F':
